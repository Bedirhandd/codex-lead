...
After cloning the repository, run `npm install` at the project root to install the Husky hooks into `.husky/`. This enables the pre-commit and pre-push checks for both frontend and backend files.

No separate frontend hook installation is required — the root hooks handle all staged-file checking.

Backend-only contributors still need Node.js and npm installed at the project root for the hook installation step, even if they do not work on the frontend.
